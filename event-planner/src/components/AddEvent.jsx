...
export default function AddEvent(){
    ...
    function createNewEvent(name,date,time,location,description){
        ...
        let lastId = eventList.legnth > 0 && eventList != null ? eventList[eventList.length-1].key : 0;
        let newEvent = new Event(lastId+1,name,date,time,location,description);
  ...
  const formik = useFormik({
    ...
    validate,
    onSubmit: (values) => {
      createNewEvent(values.eventName,values.date,values.time,values.location,values.description);
    },
  });
  ...
  return (
    ...
    <div style={{maxWidth: 600}}>
      ...
      <form className='form-container' onSubmit={formik.handleSubmit}>
        ...
        />
        {formik.touched.eventName && formik.errors.eventName ? <div>{formik.errors.eventName}</div> : null}
        ...
        />
        <button type="submit" disabled={!formik.dirty}>Add Event</button>
      </form>
    </div>
    </>
  );
